...
namespace smartshop.Api.Controllers
{
    ...
    public class SuppliersController : ControllerBase
    {
        ...
        public ActionResult<PaginationResponse<SupplierDto>> Get([FromQuery] PaginateQueryParams queryParams)
        {
            try
            {
        ...
        [ProducesResponseType(500)]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Policy = AuthPolicies.PURCHASE_MANAGEMENT)]
        public ActionResult Post([FromBody] SupplierViewModel model)
        {
            try
            {
        ...
        [ProducesResponseType(500)]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Policy = AuthPolicies.PURCHASE_MANAGEMENT)]
        public ActionResult Edit([FromBody] SupplierViewModel model, int id)
        {
            try
            {
                var businessId = User.GetBusinessId();
